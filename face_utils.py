try:
    ...
    # Optional dependency: app can still run without face recognition features.
    face_recognition = None
import numpy as np
import cv2
import base64
from models import db, FaceEncoding, Settings
...
def _find_face_locations(rgb_image):
    """Run face detection with a few fallback strategies for low-light webcam frames."""
    # First pass: fastest path.
    locations = face_recognition.face_locations(rgb_image, number_of_times_to_upsample=1, model='hog')
    if locations:
        return locations

    # Fallback pass: improve contrast in dark images.
    gray = cv2.cvtColor(rgb_image, cv2.COLOR_RGB2GRAY)
    enhanced = cv2.equalizeHist(gray)
    enhanced_rgb = cv2.cvtColor(enhanced, cv2.COLOR_GRAY2RGB)
    locations = face_recognition.face_locations(enhanced_rgb, number_of_times_to_upsample=1, model='hog')
    if locations:
        return locations

    # Final pass: more aggressive upsampling for smaller/distant faces.
    return face_recognition.face_locations(rgb_image, number_of_times_to_upsample=2, model='hog')
